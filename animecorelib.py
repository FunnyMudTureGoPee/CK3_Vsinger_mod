...
# --- 遗传概率配置 (0-100) ---
N_FATHER_V_MOTHER_FEMALE_CHANCE = 100
N_FATHER_V_MOTHER_MALE_CHANCE = 25
V_FATHER_V_MOTHER_FEMALE_CHANCE = 100
V_FATHER_V_MOTHER_MALE_CHANCE = 50
V_FATHER_N_MOTHER_FEMALE_CHANCE = 60
V_FATHER_N_MOTHER_MALE_CHANCE = 40
# --- 配置区结束 ---
...
def main_handler():
    ...
    print("  > Generated inheritance script files with custom fallback logic.")
...
def generate_inheritance_files():
    """
    Generates all inheritance-related script files with custom fallback logic.
    """
    ...
    with open(lineage_triggers_file, "w", encoding=code) as f:
        ...
        all_traits = {trait for traits in lineage_dict.values() for trait in traits}
    ...
    with open(inheritance_helpers_file, "w", encoding=code) as f:
        ...
        f.write("\t# --- First, calculate the weight for each lineage ---\n")
        for lineage in lineages:
            f.write(f"\tset_variable = {{ name = {lineage}_weight value = 0 }}\n")
            f.write(f"\tif = {{\n")
            f.write(f"\t\tlimit = {{ scope:mother ?= {{ has_{lineage}_lineage_trait = yes }} }}\n")
            f.write(f"\t\tchange_variable = {{ name = {lineage}_weight add = 100 }}\n")
            f.write(f"\t}}\n")
            f.write(f"\tif = {{\n")
            f.write(f"\t\tlimit = {{ scope:real_father ?= {{ has_{lineage}_lineage_trait = yes }} }}\n")
            f.write(f"\t\tchange_variable = {{ name = {lineage}_weight add = 100 }}\n")
            f.write(f"\t}}\n\n")
        f.write("\t# --- Then, randomly select a lineage ---\n")
        f.write("\trandom_list = {\n")
        for lineage in lineages:
            f.write(f"\t\t1 = {{\n")
            f.write(f"\t\t\tweight = var:{lineage}_weight\n")
        ...
        # --- New: Custom fallback for V-Father + V-Mother sons ---
        f.write("# This effect creates a random human appearance for sons of two Vsinger parents.\n")
        f.write("vsinger_fallback_to_random_human_effect = {\n")
        f.write("\tcreate_character = {\n")
        f.write("\t\tgender = male\n")
        f.write("\t\tage = 0\n")
        f.write("\t\tculture = root.culture\n")
        f.write("\t\tfaith = root.faith\n")
        f.write("\t\tfather = global_var:anime_dummy_male\n")
        f.write("\t\tmother = global_var:anime_dummy_female\n")
        f.write("\t\tsave_scope_as = dummy_child\n")
        f.write("\t}\n")
        f.write("\tscope:dummy_child = {\n")
        f.write("\t\tcopy_inheritable_appearance_to = root\n")
        f.write("\t\tdeath = natural\n")
        f.write("\t}\n")
        f.write("\tset_muggle_ethnicity = yes\n")
        f.write("}\n")
    ...
    with open(genetic_effects_file, "w", encoding=code) as f:
        ...
        f.write("\t\t\tlimit = {\n")
        f.write("\t\t\t\tOR = {\n")
        f.write("\t\t\t\t\tscope:real_father ?= { has_any_vsinger_trait = yes }\n")
        f.write("\t\t\t\t\tscope:mother ?= { has_any_vsinger_trait = yes }\n")
        f.write("\t\t\t\t}\n")
        f.write("\t\t\t}\n\n")
        ...
        f.write("\t\t\tif = {\n")
        f.write("\t\t\t\tlimit = {\n")
        f.write("\t\t\t\t\tNOT = { scope:real_father ?= { has_any_vsinger_trait = yes } }\n")
        f.write("\t\t\t\t\tscope:mother ?= { has_any_vsinger_trait = yes }\n")
        f.write("\t\t\t\t}\n")
        f.write("\t\t\t\tif = {\n")
        f.write("\t\t\t\t\tlimit = { is_female = yes }\n")
        f.write(f"\t\t\t\t\trandom_list = {{\n")
        f.write(f"\t\t\t\t\t\t{N_FATHER_V_MOTHER_FEMALE_CHANCE} = {{\n")
        for i, lineage in enumerate(lineages):
            clause = "if" if i == 0 else "else_if"
            f.write(f"\t\t\t\t\t\t\t{clause} = {{ limit = {{ scope:mother ?= {{ has_{lineage}_lineage_trait = yes }} }} vsinger_inherit_{lineage}_lineage_effect = yes }}\n")
        f.write(f"\t\t\t\t\t\t}}\n")
        if N_FATHER_V_MOTHER_FEMALE_CHANCE < 100:
            f.write(f"\t\t\t\t\t\t{100 - N_FATHER_V_MOTHER_FEMALE_CHANCE} = {{ anime_on_birth_set_portrait_fallback = yes }}\n")
        f.write("\t\t\t\t\t}\n")
        f.write("\t\t\t\t}\n")
        f.write("\t\t\t\telse = {\n")
        f.write(f"\t\t\t\t\trandom_list = {{\n")
        f.write(f"\t\t\t\t\t\t{N_FATHER_V_MOTHER_MALE_CHANCE} = {{\n")
        for i, lineage in enumerate(lineages):
            clause = "if" if i == 0 else "else_if"
            f.write(f"\t\t\t\t\t\t\t{clause} = {{ limit = {{ scope:mother ?= {{ has_{lineage}_lineage_trait = yes }} }} vsinger_inherit_{lineage}_lineage_effect = yes }}\n")
        f.write(f"\t\t\t\t\t\t}}\n")
        f.write(f"\t\t\t\t\t\t{100 - N_FATHER_V_MOTHER_MALE_CHANCE} = {{ anime_on_birth_set_portrait_fallback = yes }}\n")
        f.write("\t\t\t\t\t}\n")
        f.write("\t\t\t\t}\n")
        f.write("\t\t\t}\n")
        ...
        f.write("\t\t\t\t\tlimit = { is_female = yes }\n")
        f.write(f"\t\t\t\t\trandom_list = {{\n")
        f.write(f"\t\t\t\t\t\t{V_FATHER_V_MOTHER_FEMALE_CHANCE} = {{ vsinger_inherit_from_parents_effect = yes }}\n")
        if V_FATHER_V_MOTHER_FEMALE_CHANCE < 100:
            f.write(f"\t\t\t\t\t\t{100 - V_FATHER_V_MOTHER_FEMALE_CHANCE} = {{ vsinger_fallback_to_random_human_effect = yes }}\n")
        f.write(f"\t\t\t\t\t}}\n")
        f.write("\t\t\t\t}\n")
        f.write("\t\t\t\telse = {\n")
        f.write(f"\t\t\t\t\trandom_list = {{\n")
        f.write(f"\t\t\t\t\t\t{V_FATHER_V_MOTHER_MALE_CHANCE} = {{ vsinger_inherit_from_parents_effect = yes }}\n")
        f.write(f"\t\t\t\t\t\t{100 - V_FATHER_V_MOTHER_MALE_CHANCE} = {{ vsinger_fallback_to_random_human_effect = yes }}\n")
        f.write(f"\t\t\t\t\t}}\n")
        f.write("\t\t\t\t}\n")
        f.write("\t\t\t}\n")
        ...
        f.write("\t\t\t\tlimit = {\n")
        f.write("\t\t\t\t\tscope:real_father ?= { has_any_vsinger_trait = yes }\n")
        f.write("\t\t\t\t\tNOT = { scope:mother ?= { has_any_vsinger_trait = yes } }\n")
        f.write("\t\t\t\t}\n")
        f.write("\t\t\t\tif = {\n")
        f.write("\t\t\t\t\tlimit = { is_female = yes }\n")
        f.write(f"\t\t\t\t\trandom_list = {{\n")
        f.write(f"\t\t\t\t\t\t{V_FATHER_N_MOTHER_FEMALE_CHANCE} = {{ vsinger_inherit_any_lineage_effect = yes }}\n")
        f.write(f"\t\t\t\t\t\t{100 - V_FATHER_N_MOTHER_FEMALE_CHANCE} = {{ anime_on_birth_set_portrait_fallback = yes }}\n")
        f.write(f"\t\t\t\t\t}}\n")
        f.write("\t\t\t\t}\n")
        f.write("\t\t\t\telse = {\n")
        f.write(f"\t\t\t\t\trandom_list = {{\n")
        f.write(f"\t\t\t\t\t\t{V_FATHER_N_MOTHER_MALE_CHANCE} = {{ vsinger_inherit_any_lineage_effect = yes }}\n")
        f.write(f"\t\t\t\t\t\t{100 - V_FATHER_N_MOTHER_MALE_CHANCE} = {{ anime_on_birth_set_portrait_fallback = yes }}\n")
        f.write(f"\t\t\t\t\t}}\n")
        f.write("\t\t\t\t}\n")
        f.write("\t\t\t}\n")
